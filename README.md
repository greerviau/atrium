...
- **A real code editor.** Syntax highlighting, search, split panes, and draggable tabs that move between panes or create splits.
- **Tabular data inspection.** CSV, TSV, and Parquet files open in a queryable data grid with configurable pagination.
- **Built-in image viewing.** Common image formats open directly in centered, fit-to-pane previews.
- **Clickable file paths in terminal output.** `nested/spec.md:42` opens at that line even when it is only a uniquely matching suffix of the workspace path. A path outside the open project — an absolute path, a `~/` path, or one printed after the shell has `cd`'d elsewhere — opens too, as an external tab; the unique-partial-suffix search stays scoped to the open project.
