...
describe("minimap Overlay window-listener leak fix", () => {
  ...
  it("does not accumulate window mouseup/mousemove/keydown/blur listeners across repeated tab close/reopen", async () => {
    ...
    vi.spyOn(window, "addEventListener").mockImplementation((type, listener, opts) => {
      if (type === "mouseup" || type === "mousemove" || type === "keydown" || type === "blur") {
    ...
    expect(registered.blur.length).toBe(0);
  });
});
