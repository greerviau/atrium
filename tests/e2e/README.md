...
3. Split the active terminal pane, run a distinct command in each of the two resulting panes, confirm neither pane's output leaks into the other's, then close one pane and confirm the other survives with its output intact.
4. Open the project-wide search overlay via Cmd/Ctrl+Shift+F, search for a string that matches `note.md`, click the result, and confirm the overlay closes and the editor jumps to `note.md`.
5. Confirm the bottom status bar shows the active file's path and cursor position and updates as the caret moves, that its search button opens the search overlay, and that its explorer/terminal toggle buttons show and hide their respective panels.
6. (Follow-up, not yet in the spec) feed a synthetic buffer containing a PR URL and a real file path and verify both linkify and that clicking the file path opens it in the editor.
...
`specs/scrollJump.e2e.js` covers issue #311 (rendered markdown scrolling back up while the user scrolls down): open `scrollJump.md` (a 140-section fixture mixing images, GFM tables, and Mermaid diagrams throughout, per the reporter's description of the file this happened on), scroll continuously from top to bottom via batches of real wheel actions chained into single W3C Actions calls (to keep consecutive ticks back-to-back rather than picking up idle time from a Node↔driver round trip between each one), sample `scrollTop` on every animation frame for the whole gesture, and assert it never moves backward by more than a couple of pixels of rounding jitter. Source-mode scrolling isn't covered here because the bug is specific to rendered mode's decoration/widget machinery, which source mode doesn't have. This spec is the reproduction and regression guard for the actual user-visible behavior; a scripted WebDriver gesture still can't fully reproduce a human's uneven scroll timing, so a manual, trackpad-driven pass on real hardware remains a useful complement, not a replacement, for this spec.
...
`specs/scrollJump.e2e.js` carries the same caveat and has not been run against a real WebView: the sandbox this was developed in had no display server at first, and once `Xvfb` was installed to provide one, the debug Tauri build the suite depends on couldn't complete in the disk space available — so the reproduction this spec is meant to confirm is still open. Run it on a real machine — macOS first, per the issue's likely platform, then Linux/WebKitGTK as a second target if it doesn't reproduce there — before treating its pass/fail result as meaningful.
