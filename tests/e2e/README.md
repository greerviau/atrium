...
`specs/smoke.e2e.js` covers the three scenarios named in the plan:
...
The native folder-picker dialog lives outside the WebView, so the spec registers the workspace root directly through the same `workspace_set_root` command the picker's callback would call, rather than trying to drive the OS dialog — everything downstream exercises real app code.
