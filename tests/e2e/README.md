...
It clicks *inside* the pane's post-scroll settle window deliberately, since that window is what both regressions are about, and reads everything it asserts on after the pane has settled.
It confirms the click actually landed mid-settle and actually registered, that the scroll position held, and that the click resolved within two source lines of where a click on the same point lands once the pane has settled.
