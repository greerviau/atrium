...
`smoke.e2e.js` has been run once, under Xvfb. 3 of its 12 scenarios pass: "markdown live preview" (open a workspace, resolve `note.md` in the file tree, and render its heading), and both syntax-highlighting scenarios (`config.toml`, `main.tf`) — each of these exercises `openWorkspace` and the file-tree `name` lookup end to end, which is what this fix targets. The remaining 9 scenarios fail for reasons unrelated to this fix (terminal-pane interactability, the search overlay not appearing, and a status-bar block that cascades from those) — tracked in issue #421.
