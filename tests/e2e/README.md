...
`specs/scrollThenClick.e2e.js` targets the rendered-markdown scroll-then-click condition issues #183 and #367 were originally filed against.
It opens `long.md` on a pane that has never yet been clicked into, wheel-scrolls it, and clicks a fixed point in the viewport deliberately *inside* the pane's post-scroll settle window — before any heading has rendered and before CodeMirror's scroll correction has landed — then reads everything it asserts on only after the pane has settled.
...
It confirms the click actually landed mid-settle and actually registered, that the scroll position held, and that the click resolved within two source lines of where a click on the same point lands once the pane has settled — not the same line exactly, since the mid-settle click's resolution reproducibly differs from its settled one by one line (issue #454).
Be clear about what that does *not* establish: mutation testing (stubbing each guard in `src/lib/editor/baseExtensions.ts` to `return false`) shows this spec passing with **both** guards deleted, because on current `main` the #183 and #367 symptoms no longer reproduce when the guards are removed — the scroll never collapses and the resolved line moves by less than this spec's own run-to-run variance.
So this is coverage of the scroll-then-click path staying sane end to end, not a fault-injection regression test for either guard; a green run here does not mean those guards are protected.
Fault-injection coverage for them belongs at the unit level, where the deferral path can be driven directly.
