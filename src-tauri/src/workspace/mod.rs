...
pub struct FsChangeEvent {
    ...
    /// from. Populated when the backend pairs both halves or `fs_watch` finds
    /// one uniquely missing path with the destination's filesystem identity;
    /// a rename that cannot be paired is surfaced as a plain `Remove` or
    /// `Create`, never as a path-only guess.
